...
public class MetodoSucursal extends Conexion {
    ...
    public void modificarSucursal(int codSucur, String calle, int codPost, String ciudad) {

        try {
            Connection conn = this.getConnection();
            String q = "{call MODIFICAR_SUCURSAL (" + codSucur + ",'" + calle + "'," + codPost + ",'" + ciudad + "')}";
            CallableStatement storedProc = conn.prepareCall(q);
            storedProc.execute();
            storedProc.close();
        ...
        public void borrarSucursal(int codSucur) {

        try {
            Connection conn = this.getConnection();
            String q = "{call BORRAR_SUCURSAL (" + codSucur + ")}";
            CallableStatement storedProc = conn.prepareCall(q);
            storedProc.execute();
            storedProc.close();
